...
public class Controller extends GameController implements ActionListener {
	...
	Label m_cooldownPopLabel, m_cooldownWizzLabel;
...
//		west.setLayout(new BoxLayout(west, BoxLayout.X_AXIS));
		...
		JPanel cooldowns = new JPanel();
		cooldowns.setLayout(new GridBagLayout());
		...
		m_cooldownPopLabel = new Label("", Label.CENTER);
		...
		cooldowns.add(m_cooldownPopLabel);
		cooldowns.add(m_cooldownWizzLabel);
		westInside.add(cooldowns);
...
//		west.add(Box.createRigidArea(new Dimension(Options.UI_HORIZONTAL_MARGIN, 0)));
	...
	public void updateCooldownsUI() {
		m_cooldownPopLabel.setText(String.format("pop: %.1f%ns", (float) m_model.getPlayer().getTimeLeftPop() / 1000));
		m_cooldownWizzLabel
